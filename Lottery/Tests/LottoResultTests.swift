...
final class LottoResultTests: XCTestCase {
    ...
    func test_NumbersFromString_StringWithoutNumbers() {
        ...
        XCTAssertThrowsError(try LottoDrawResult.numbersFromString(string)) { error in
    ...
    func test_NumbersFromString_CorrectString() {
        ...
        // when
        if let result = try? LottoDrawResult.numbersFromString(string) {
            // then
    ...
    func test_ResultsFromLines_CorrectLine() {
        ...
        // when
        if let result = try? LottoDrawResult.resultsFrom(lines: [line]) {
            // then
    ...
    func test_ResultsFromLines_EmptyLine() {
        ...
        let line = ""
        // when
        XCTAssertThrowsError(try LottoDrawResult.resultsFrom(lines: [line])) { error in
            // then
